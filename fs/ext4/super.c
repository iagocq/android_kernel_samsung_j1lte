				...
				, int start, int len)
{
	if (bh) {
		printk(KERN_ERR " print_bh: bh %p,"
				...
				(struct buffer_head *) bh, (unsigned int) bh->b_size, (void *) bh->b_data);
						...
						, unsigned char *data_to_dump, int start, int len)
{
	...
	printk(KERN_ERR " [partition info] s_id : %s, start block# : %lu\n"
			, sb->s_id, sb->s_bdev->bd_part->start_sect);
	printk(KERN_ERR " dump block# : %lu, start offset(byte) :", blocknr);
