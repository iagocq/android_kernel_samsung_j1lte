...
#ifdef CONFIG_MACH_PXA_SAMSUNG
	pdata->n_wakeup_src = n_wakeup_src =
				of_gpio_named_count(node, "edge-wakeup-gpios");
	if (n_wakeup_src <= 0)
		dev_err(dev, "No edge-wakeup-gpios defined\n");

	gpios = devm_kzalloc(dev, sizeof(unsigned int) * n_wakeup_src,
				GFP_KERNEL);
	if (!gpios) {
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->n_wakeup_src; i++) {
		if (pdata->edge_wakeup_gpio_tbl[i] >= 0) {
			ret = request_mfp_edge_wakeup(
		...
		if (ret)
			...
			dev_info(dev,
				"gpio %d is requested as edge wakeup source\n",
				pdata->edge_wakeup_gpio_tbl[i]);
		}
	}
#endif
...
static int gpio_keys_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < ddata->pdata->n_wakeup_src; i++) {
		if (ddata->pdata->edge_wakeup_gpio_tbl[i] >= 0) {
			ret = remove_mfp_edge_wakeup(
					ddata->pdata->edge_wakeup_gpio_tbl[i]);
			if (ret)
				dev_err(&pdev->dev,
					"Failed to remove edge wakeup"
					" for gpio_key[%d]\n",
					ddata->pdata->edge_wakeup_gpio_tbl[i]);
		}
	}
