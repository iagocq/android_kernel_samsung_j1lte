...
static ssize_t mms_dev_fs_write(struct file *fp, const char *wbuf, size_t cnt, loff_t *fpos)
{
	...
	if (!buf || copy_from_user(buf, wbuf, cnt)) {
...
#if MMS_HEX_DEBUG
		print_hex_dump(KERN_ERR, MMS_DEVICE_NAME" : input ", DUMP_PREFIX_OFFSET,
						16, 1, wbuf, cnt, false);
		print_hex_dump(KERN_ERR, MMS_DEVICE_NAME" : output ", DUMP_PREFIX_OFFSET,
						16, 1, info->dev_fs_buf, buf[cnt - 2], false);
#endif
	} else if (cmd == 2) {
...
* Process table data
*/
static int mms_proc_table_data(struct mms_ts_info *info, u8 size, u8 data_type_size,
						...
						u8 col_num, u8 buf_col_num, u8 rotate, u8 key_num)
{
	...
	for (i_y = 0; i_y < lim_y; i_y++) {
		...
		for (i_x = 0; i_x < lim_x; i_x++) {
			switch (data_size) {
			case 1:
				printk(KERN_INFO" %3d", info->image_buf[i_y * max_x + i_x]);
				sprintf(data, " %3d", info->image_buf[i_y * max_x + i_x]);
				break;
			case 2:
				printk(KERN_INFO" %5d", info->image_buf[i_y * max_x + i_x]);
				sprintf(data, " %5d", info->image_buf[i_y * max_x + i_x]);
				break;
			case 4:
				printk(KERN_INFO" %6d", info->image_buf[i_y * max_x + i_x]);
...
static DEVICE_ATTR(mode_charger, S_IRUGO | S_IWUSR | S_IWGRP, mms_sys_charger_mode_show, mms_sys_charger_mode_store);
static DEVICE_ATTR(mode_cover_window, S_IRUGO | S_IWUSR | S_IWGRP, mms_sys_window_mode_show, mms_sys_window_mode_store);
...
int mms_sysfs_create(struct mms_ts_info *info)
{
	...
	if (sysfs_create_link(NULL, &client->dev.kobj, MMS_DEVICE_NAME)) {
		dev_err(&client->dev, "%s [ERROR] sysfs create link\n", __func__);
		return -EAGAIN;
	}

	info->print_buf = kzalloc(sizeof(u8) * 4096, GFP_KERNEL);
	info->image_buf = kzalloc(sizeof(int) *
			((info->node_x * info->node_y) + info->node_key), GFP_KERNEL);
