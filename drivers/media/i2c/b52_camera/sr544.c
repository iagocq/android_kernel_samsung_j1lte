...
static int SR544_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		/* 5M XShutDown Enable*/
		if (power->pwdn)
			gpiod_set_value_cansleep(power->pwdn, 0);//pin value is high
		...
		/* 5M Reset Enable*/	
		if (power->rst)
			gpiod_set_value_cansleep(power->rst, 0);//pin value is high
		...
		msleep(10);
		if (power->rst)
			gpiod_set_value_cansleep(power->rst, 1);//pin value is low
			
		msleep(15);
		
		clk_disable_unprepare(sensor->clk);/*mclk disable*/

		msleep(5);		

		if (power->pwdn)
			gpiod_set_value_cansleep(power->pwdn, 1);//pin value is low
