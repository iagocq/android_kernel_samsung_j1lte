    ...
    )
{
    ...
    for (map = Mmu->mapLogical, gotIt = gcvFALSE; !gotIt;)
    {
        ...
        for (; !gotIt && (index < Mmu->pageTableEntries);)
        {
            ...
            default:
                gcmkTRACE(gcvLEVEL_ERROR, "MMU table corrupted at index %u, recollect to fix!", index);
                forceRecollect = gcvTRUE;
            }

            if(forceRecollect)
            {
                /* force recollect free entries, and reset Heaplist */
