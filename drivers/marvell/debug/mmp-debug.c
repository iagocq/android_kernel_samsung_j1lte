...
#define DVC_HELD_STATUS		0xB8
#define FCDONE_HELD_STATUS	0xC0
#define PMULPM_HELD_STATUS	0xC8
#define CORELPM_HELD_STATUS	0xD0
...
{
	recorded_helds.fabws = readl_relaxed(squ_base + FABTIMEOUT_HELD_WSTATUS);
	recorded_helds.fabrs = readl_relaxed(squ_base + FABTIMEOUT_HELD_RSTATUS);
...
static int axi_timeout_probe(struct platform_device *pdev)
{
	u32 tmp, modes;
	int irq, i, ret = 0;
	struct resource *res;
