...
#if 0
static int pm886_gpadc_bias_current(struct pm886_gpadc_info *info,
						int channel,
						int *bias_voltage, int bias_current)
{
	int ret, data;
	u8 reg, gpadc_number;
	gpadc_number = info->channel_to_gpadc_num(channel);
	...
	case GPADC_3_RES:
		...
		break;
	default:
		dev_err(info->chip->dev, "%s: unsupported gpadc!(%d)\n",
			__func__, gpadc_number);
		return -EINVAL;
	}
	...
	if (bias_current > 76)
		bias_current = 76;
	else if (bias_current < 1)
		bias_current = 1;
	
	data = (bias_current - 1) / 5;
	ret = regmap_update_bits(info->chip->gpadc_regmap, reg, 0xf, data);
	if (ret < 0)
		return ret;

	ret = pm886_gpadc_get_processed(info, channel, bias_voltage);
	*bias_voltage = (int)(*bias_voltage / 1000);
	if (ret < 0)
		return ret;
...
static int pm886_gpadc_setup(struct pm886_gpadc_info *gpadc)
{
	...
	regmap_write(gpadc->chip->gpadc_regmap, PM886_GPADC_CONFIG2, 0xfd);
