...
static void __iomem *mfp_base = NULL;

static void sdhci_mfp_dump(void)
{
	if (mfp_base == NULL)
		return;
	printk("======dump eMMC MFP setting======\n");
	printk("NDIO_0 = 0x%x\n", readl(mfp_base + 0x40));
	printk("NDIO_1  = 0x%x\n", readl(mfp_base + 0x3C));
	printk("NDIO_2  = 0x%x\n", readl(mfp_base + 0x38));
	printk("NDIO_3  = 0x%x\n", readl(mfp_base + 0x34));
...
static void sdhci_dumpregs(struct sdhci_host *host)
{
	...
	for (i = 0x100; i <= 0x11c; i = i + 4) {
		pr_info(DRIVER_NAME "offset 0x%x = 0x%x\n", i, sdhci_readl(host, i));
	}
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	mmc = host->mmc;
		...
		if (mfp_base == NULL)
			mfp_base = ioremap(0xD401E000, 512);
